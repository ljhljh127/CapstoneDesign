...
class Inferer:
    ...
    def infer(self, conf_thres, iou_thres, classes, agnostic_nms, max_det, save_dir, save_txt, save_img, hide_labels, hide_conf, view_img=True, min_objects_inside=45):

        def check_overlap(box1, box2, overlap_threshold=0.7):
            # Calculate the coordinates of the intersection (overlapping area) between two boxes
            ...
            # Check if the overlap ratio is above the specified threshold
            ...
            if len(det):
                ...
                for *xyxy, conf, cls in reversed(det):
                    ...
                    if save_img:
                        ...
                        # Calculate the coordinates for the 5x enlarged bounding box
                        enlarged_p1 = (int(xyxy[0] - (xyxy[2] - xyxy[0]) * 4), int(xyxy[1] - (xyxy[3] - xyxy[1]) * 4))
                        enlarged_p2 = (int(xyxy[2] + (xyxy[2] - xyxy[0]) * 4), int(xyxy[3] + (xyxy[3] - xyxy[1]) * 4))

                        # Count objects inside the "key" bounding box
                        num_objects_inside = 0
                        for *xyxy2, _, _ in reversed(det):
                            if check_overlap(enlarged_p1 + enlarged_p2, xyxy2):
                                num_objects_inside += 1
                                

                        if num_objects_inside >= min_objects_inside:
    ...
    def plot_box_and_label(image, lw, box, label='', color=(128, 128, 128), txt_color=(255, 255, 255), font=cv2.FONT_HERSHEY_COMPLEX):
        ...
        # Calculate the coordinates for the 5x enlarged bounding box
        enlarged_p1 = (int(box[0] - (box[2] - box[0]) * 4), int(box[1] - (box[3] - box[1]) * 4))
        enlarged_p2 = (int(box[2] + (box[2] - box[0]) * 4), int(box[3] + (box[3] - box[1]) * 4))
        
        # Draw the 5x enlarged bounding box with a different color
        cv2.rectangle(image, enlarged_p1, enlarged_p2, (255, 0, 0), thickness=lw, lineType=cv2.LINE_AA)
